...
def count_tokens(text):
    """
    原生轻量级 Token 估算：
    中文/中文字符每个算 1 个 Token；英文单词按空格切分每个算 1 个 Token。
    """
    tokens = 0
    for char in text:
        if '\u4e00' <= char <= '\u9fff':
...
"""
    ...
    print(f"已生成 {len(documents)} 个合规切片，正在调用 nomic-embed-text 写入本地 ChromaDB...")
...
def process_file(file_path, collection, progress_prefix=""):
    ...
    response_stream = ollama.generate(
        ...
        options={
            "temperature": 0.1,
            "num_ctx": 8192,  # 增大上下文窗口到 8K，避免推理链条长时被阶段截断
