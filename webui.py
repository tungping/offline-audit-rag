...
SEMICONDUCTOR_DEMO_TEXT = """【Synthetic Demo，仅用于功能演示】
一种 SiC MOSFET 器件，包括碳化硅外延层、沟槽栅、栅介质层、源区、体区和漏极电极。
沟槽底部设置屏蔽区，用于降低栅氧电场并改善器件可靠性，适用于高压功率半导体开关。
"""


def demo_text_for_mode(mode: str) -> str:
    if mode == app.SEMICONDUCTOR_IP_MODE:
        return SEMICONDUCTOR_DEMO_TEXT
    return DEMO_TEXT


def input_label_for_mode(mode: str) -> str:
    if mode == app.SEMICONDUCTOR_IP_MODE:
        return "公开专利文本 / 技术交底 / 产品说明 / 论文摘要"
    return "会议记录 / SOP / 任务指派文本"
...
def render_running_audit_status(info_func=st.info) -> None:
    # Avoid st.spinner here: reruns remount the animated element and make it flicker.
    info_func("正在审计中，可以点击“停止审计”中断本次任务。")
...
def render_history() -> None:
    ...
    mode_df = pd.DataFrame(
        [{"mode": mode, "count": count} for mode, count in summary.get("mode_counts", {}).items()]
    )
    if not mode_df.empty:
        st.subheader("分析模式分布")
        mode_df = mode_df.sort_values("count", ascending=False)
...
def render_classic_workflow() -> None:
    ...
    with input_tab:
        pasted_text = st.text_area(
            input_label_for_mode(selected_mode),
            value=demo_text_for_mode(selected_mode),
            height=220,
            key=f"pasted_text_{selected_mode}",
        )
